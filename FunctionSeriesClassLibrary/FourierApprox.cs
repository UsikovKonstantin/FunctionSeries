...
public class FourierApprox
{
    ...
    private readonly int num_points; // Количество точек в аппроксимации
    private readonly double period; // Период ряда 
    private readonly List<(double x, double y)> Point_cloud; //Поле точек для аппроксимации

    /// <summary>
    ///     Получение ряда Фурье посредством поля точек (Тип 2)
    /// </summary>
    /// <param name="point_cloud">Поле точек</param>
    /// <param name="type">Тип преобразования: стандартный или быстрый</param>
    ...
    {
        var Dk = -Complex.ImaginaryOne * (2 * Math.PI / period) / num_points;
        for (var i = 0; i < num_points / 2; i++)
        {
            var Factor = Complex.Pow(new Complex(Math.E, 0),
                Dk * i);
            Frequencies.Add(even.Frequencies[i] + odd.Frequencies[i] * Factor);
        }

        for (var i = num_points / 2; i < num_points; i++)
        {
            var Factor = Complex.Pow(new Complex(Math.E, 0),
                Dk * i);
    ...
    /// </summary>
    private void NDFT_Coefficients()
    {
        var Dk = -Complex.ImaginaryOne * (2 * Math.PI / period);
        for (var i = 0; i < num_points; i++) Frequencies.Add(Get_Specific(i, Dk));
    }
    ...
    private Complex Get_Specific(int i, Complex dk)
    {
        ...
        foreach (var Point in Point_cloud) Coef += Point.y * Complex.Pow(new Complex(Math.E, 0), dk * i * Point.x);
    ...
    public double Compute(double x, int Precision)
    {
        Complex result = 0;
        var Dk = Complex.ImaginaryOne * (2 * Math.PI / period);
        result += Frequencies[0] * Complex.Pow(new Complex(Math.E, 0), 0);
        for (var i = 1; i < Precision; i++)
        {
            result += Frequencies[i] * Complex.Pow(new Complex(Math.E, 0), Dk * i * x);
            Complex t = new(Frequencies[i].Real, -Frequencies[i].Imaginary);
            result += t * Complex.Pow(new Complex(Math.E, 0), Dk * -i * x);
        }
    ...
    public override string ToString()
    {
        ...
        foreach (var frequency in Frequencies) builder.Append($"{frequency}, ");
